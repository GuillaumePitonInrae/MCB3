...
PLOT_INPUTnew<-function (input, N = 1000, mode = "IND") 
{
  ...
  # dev.new()
  par(mfrow = c(nrow, ncol), cex = 1.1, mex = 0.75, mar = c(4.5,4.005, 2, 1))
  ndiscrprob = 1000
    ...
    if (inp$type == "possi") {
      ...
      if (("possi" %in% subtype) & (!("proba" %in% subtype)) & 
        ...
        plot(ecdf(Xr[1, ]), xlim = c(min(knots(ecdf(Xr[1, 
                                                       ]))), max(knots(ecdf(Xr[2, ])))), col = "blue", 
             cex = 1, lwd = 2, xlab = inp$name, ylab = "CDF", 
             main = inp$type)
        lines(ecdf(Xr[2, ]), col = "blue", cex = 1, 
              lwd = 2)
      }
